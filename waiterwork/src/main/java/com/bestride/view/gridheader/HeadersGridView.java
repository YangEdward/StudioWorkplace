...
package com.bestride.view.gridheader;
...
import com.bestride.view.gridheader.HeadersBaseAdapterWrapper.HeaderFillerView;
 ...
 * @author Edward
 */
public class HeadersGridView extends GridView implements OnScrollListener,
    ...
    static final String TAG = HeadersGridView.class.getSimpleName();
    ...
    protected HeadersBaseAdapterWrapper mAdapter;
    ...
    public HeadersGridView(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.gridViewStyle);
    }

    public HeadersGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        HeadersBaseAdapter baseAdapter;
//        if (adapter instanceof HeadersBaseAdapter) {
//            baseAdapter = (HeadersBaseAdapter)adapter;
//        } else if (adapter instanceof HeadersSimpleAdapter) {
            // Wrap up simple adapter to auto-generate the data we need.
            baseAdapter = new HeadersSimpleAdapterWrapper(
                    (HeadersSimpleAdapter)adapter);
//        } else {
//            // Wrap up a list adapter so it is an adapter with zero headers.
//            baseAdapter = new HeadersListAdapterWrapper(adapter);
//        }

        this.mAdapter = new HeadersBaseAdapterWrapper(getContext(), this, baseAdapter);
    ...
    private int findMotionHeader(float y) {
        ...
        for (int i = getFirstVisiblePosition(); i <= getLastVisiblePosition();) {
            ...
            if (id == HeadersBaseAdapterWrapper.ID_HEADER) {
    ...
    private void scrollChanged(int firstVisibleItem) {
        ...
        if (childCount != 0) {
            ...
            for (int i = 0; i < childCount; i += mNumMeasuredColumns) {
                ...
                if (mAdapter.getItemId(getPositionForView(child)) == HeadersBaseAdapterWrapper.ID_HEADER
    ...
    static class SavedState extends BaseSavedState {
        ...
        public String toString() {
            return "HeadersGridView.SavedState{"
